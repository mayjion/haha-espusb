        ...
        for (int i = 0; i < MAX_CLIENTS; i++) {
            if (clients[i].active && clients[i].sock >= 0) {
                ...
                while (remaining > 0 && send_success && retry_count < max_retries) {
                    ...
                    if (select(clients[i].sock + 1, NULL, &writefds, NULL, &tv) > 0 && FD_ISSET(clients[i].sock, &writefds)) {
                        // Writable: try send
                        ...
                        if (sent < 0) {
                            ...
                            if (err == EAGAIN || err == EWOULDBLOCK) {
                                ...
                                ESP_LOGW(TAG, "%s TCP send error for client %d: %s (errno:%d), closing", log_prefix, i, strerror(err), err);
                    ...
                    // Safety: Close if stalled >100ms
                    if ((xTaskGetTickCount() - stall_start) > pdMS_TO_TICKS(100)) {
                        ESP_LOGW(TAG, "%s Client %d stalled >100ms, closing", log_prefix, i);
                        close(clients[i].sock);
                        clients[i].sock = -1;
                        clients[i].active = false;
                        num_active_clients--;
                ...
                if (!send_success || total_sent < len) {
                    ESP_LOGW(TAG, "%s Failed to send %zu/%zu to client %d (retries:%d)", 
            ...
            if (num_active_clients < MAX_CLIENTS) {
                ...
                if (sock >= 0) {
                    ...
                    if (slot != -1) {
                        ...
                        // 自定义keepalive参数（ESP-IDF支持）
                        int idle = 10;  // 10秒无数据后开始发送keepalive探针
                        setsockopt(sock, IPPROTO_TCP, TCP_KEEPIDLE, &idle, sizeof(idle));
                        int interval = 5;  // 每5秒发送一个探针
                        setsockopt(sock, IPPROTO_TCP, TCP_KEEPINTVL, &interval, sizeof(interval));
                        int count = 3;  // 连续3个探针无响应后关闭连接
                        setsockopt(sock, IPPROTO_TCP, TCP_KEEPCNT, &count, sizeof(count));

                        ESP_LOGI(TAG, "Keepalive enabled for client %d (idle=%ds, intvl=%ds, cnt=%d)", slot, idle, interval, count);
...
void app_main(void) {
    ...
    xTaskCreate(tcp_server_task, "tcp_server", 8192, NULL, 8, NULL);  // 栈大小增加到4096
