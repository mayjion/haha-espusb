...
#define LED_PIN GPIO_NUM_8  // ESP32-C3 typical LED pin
#define BUTTON_PIN GPIO_NUM_9  // ESP32-C3 typical button pin
...
static uint8_t debug_mode = 1; // Enabled for debugging
...
#define CDC_LOG(isdebug, format, ...) do { \
    ...
    if (esp_get_free_heap_size() < MIN_HEAP_THRESHOLD) { \
...
static esp_err_t init_system(void)
{
    ...
    size_t free_heap = esp_get_free_heap_size();
    CDC_LOG(1, "Starting USB CDC with WiFi and TCP server, free heap: %zu\n", free_heap);
    if (free_heap < MIN_HEAP_THRESHOLD) {
...
{
    CDC_LOG(1, "USB CDC initialization\n");
...
void app_main(void)
{
    ...
    CDC_LOG(1, "USB CDC with WiFi and TCP server initialization DONE, free heap: %zu\n", esp_get_free_heap_size());
    app_message_t msg;
    esp_task_wdt_config_t wdt_config = {
        .timeout_ms = 10000,
        .idle_core_mask = 0,
