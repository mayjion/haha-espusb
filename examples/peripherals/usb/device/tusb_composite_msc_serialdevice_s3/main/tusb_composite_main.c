...
static void led_task(void *pvParameters) {
    ESP_LOGI(TAG, "LED task started, initial num_clients=%d", num_active_clients);  // 诊断日志
    ...
    while (1) {
        ...
        // 每 5s 打印状态（仅在无连接时，避免干扰）
        if (num_active_clients == 0 && (xTaskGetTickCount() - last_log_time) > pdMS_TO_TICKS(5000)) {
            ESP_LOGI(TAG, "LED flashing (no clients), GPIO17 level=%d", gpio_get_level(LED_GPIO));
...
void app_main(void) {
    ...
    xTaskCreate(led_task, "led_task", 2048, NULL, 3, NULL);  // 新增LED任务，低优先级，小栈
