...
#include "usbhandle.h"  // CHANGED: For tx_ring_t (now pointer-based)
#include "protocol.h"
#include "wificonfig.h"

// Defines (unchanged except added RX ring size)
...
extern rx_ring_t rx_ring;  // New: Shared RX ring for TCP -> parse task
extern SemaphoreHandle_t rx_mutex;  // New: For rx_ring protection
// tx_ring extern via usbhandle.h

void app_main(void);
void tcp_server_task(void *pvParameters);
void send_task(void *pvParameters);
void parse_and_usb_task(void *pvParameters);  // New: Parse RX ring and output to USB
esp_err_t send_reliable(client_t *client, uint8_t type, const uint8_t *payload, uint16_t payload_len);
esp_err_t rx_ring_append(const uint8_t *data, size_t len);  // New: Append to RX ring
size_t rx_ring_consume(uint8_t *buf, size_t max_len);  // New: Consume from RX ring
